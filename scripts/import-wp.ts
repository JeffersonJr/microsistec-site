...
async function downloadImage(url: string, prefix: string): Promise<string> {
  ...
  if (url.startsWith("/") || url.startsWith("data:") || url.includes("unsplash.com")) return url;
...
async function processContentImages(html: string, postId: string): Promise<string> {
...
async function run() {
  const xmlPath = join(process.cwd(), "content", "microsistec.WordPress.2026-08-12.xml");
  ...
  for (const item of postsArray) {
    ...
    let tag = "Geral";
    if (item.category) {
      let firstCat = Array.isArray(item.category) ? item.category[0] : item.category;
    ...
    let rawCoverUrl = "https://images.unsplash.com/photo-1564013799919-ab600027ffc6?auto=format&fit=crop&w=800&q=80"; // Default
    ...
    if (rawCoverUrl.includes("microsistec.com.br/wp-content/uploads/")) {
      rawCoverUrl = rawCoverUrl.replace(/https?:\/\/(www\.)?microsistec\.com\.br\/wp-content\/uploads\//, "https://d2xnmiivr6f5qr.cloudfront.net/wp-content/uploads/");
    }
  ...
  console.log(`Successfully extracted ${blogPosts.length} posts and downloaded images.`);
